package com.ej.recharge.vision.cameraview
...
class CameraActivity : AppCompatActivity(),
  View.OnClickListener, OptionView.Callback {
  ...
  private val snapCamera: CameraView by lazy { findViewById(R.id.camera) }
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    snapCamera.setLifecycleOwner(this)
    snapCamera.addCameraListener(Listener())

    if (USE_FRAME_PROCESSOR) {

      snapCamera.addFrameProcessor(object : FrameProcessor {
  ...
  private inner class Listener : CameraListener() {
    ...
    override fun onCameraOpened(options: CameraOptions) {
      ...
      for (i in 0 until group.childCount) {
        ...
        view.onCameraOpened(snapCamera, options)
      }
    }
  ...
  private fun capturePicture() {
    if (snapCamera.mode == Mode.VIDEO) return run {
      message("Can't take HQ pictures while in VIDEO mode.", false)
    }
    if (snapCamera.isTakingPicture) return
    captureTime = System.currentTimeMillis()
    message("Capturing picture...", false)
    snapCamera.takePicture()
  }

  private fun capturePictureSnapshot() {
    if (snapCamera.isTakingPicture) return
    if (snapCamera.preview != Preview.GL_SURFACE) return run {
      message("Picture snapshots are only allowed with the GL_SURFACE preview.", true)
    }
    captureTime = System.currentTimeMillis()
    message("Capturing picture snapshot...", false)
    snapCamera.takePictureSnapshot()
  }

  private fun captureVideo() {
    if (snapCamera.mode == Mode.PICTURE) return run {
      message("Can't record HQ videos while in PICTURE mode.", false)
    }
    if (snapCamera.isTakingPicture || snapCamera.isTakingVideo) return
    message("Recording for 5 seconds...", true)
    snapCamera.takeVideo(File(filesDir, "video.mp4"), 5000)
  }

  private fun captureVideoSnapshot() {
    if (snapCamera.isTakingVideo) return run {
    ...
    snapCamera.takeVideoSnapshot(File(filesDir, "video.mp4"), 5000)
  }

  private fun toggleCamera() {
    if (snapCamera.isTakingPicture || snapCamera.isTakingVideo) return
    when (snapCamera.toggleFacing()) {
  ...
  private fun changeCurrentFilter() {
    ...
    // Normal behavior:
    snapCamera.filter = filter.newInstance()
  ...
  override fun <T : Any> onValueChanged(option: Option<T>, value: T, name: String): Boolean {
    if (option is Option.Width || option is Option.Height) {
      val preview = snapCamera.preview
