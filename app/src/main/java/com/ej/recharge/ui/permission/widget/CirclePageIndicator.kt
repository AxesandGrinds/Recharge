package com.ej.recharge.ui.permission.widget
...
class CirclePageIndicator @JvmOverloads constructor(
        ...
        companion object {
            @JvmField val CREATOR: Parcelable.Creator<SavedState> =
                object : Parcelable.Creator<SavedState> {
                    override fun createFromParcel(rin: Parcel): SavedState? {
                        return SavedState(rin)
                    }
    ...
    init {
        ...
        if (background != null) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) setBackground(background) else setBackgroundDrawable(
                background
            )
        }
        a.recycle()
